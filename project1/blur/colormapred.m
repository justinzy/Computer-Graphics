...
imwrite(ColorPic1Map1,'colormapRedIntensity.png','png');
...
Lp = hsv2rgb(Lp);
% imshow(Lp);
imwrite(Lp,'colormapRedSaturation','png');
...
% imshow(rainbow);
imwrite(rainbow,'colormapRedRainbow','png');
...
for i = 1:rows
    ...
    imwrite(blueRed,'colormapRedBlueRed.png','png');
...
% imshow(twoColor);
imwrite(twoColor, 'colormapRed2color.png', 'png');
...
heat = uint8(heat);
% imshow(heat);
imwrite(heat, 'colormapRedHeat.png', 'png');
...
% imshow(colcon);
imwrite(colcon,'colormapRedcontour.png','png');
